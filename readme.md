...
You can access your application via **`localhost`**, if you're running the containers directly, or through **`192.168.33.21`** when run on a vm. nginx and mailhog both respond to any hostname, in case you want to add your own hostname on your `/etc/hosts` 

Service|Address outside containers|Address outside VM
------|---------|-----------
Webserver|[localhost:9100](http://localhost:9100)|[192.168.33.21](http://192.168.33.21)
...
------|---------|-----------
php-fpm|laravel-studio-php-fpm|9000
MySQL|laravel-studio-mysql|3306 (default)
...
**Note 2:** if you're using Vagrant, you'll need to ssh into it first
  ...
  * Execute command inside of container: `docker exec -it laravel-studio-php-fpm COMMAND` where `COMMAND` is whatever you want to run. For instance, `/bin/bash` to open a console prompt.
